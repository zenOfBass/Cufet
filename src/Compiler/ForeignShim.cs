...
public sealed class GccForeignRunner : IForeignRunner
{
    ...
    // Source declared by resultless axioms, per language, in the order it was written — a preamble
    // may name what an earlier one declared.
    private readonly Dictionary<string, List<string>> _preambles = new(StringComparer.OrdinalIgnoreCase);

    public void Declare(string language, string source, int line)
    {
        if (!_preambles.TryGetValue(language, out var forLanguage))
            _preambles[language] = forLanguage = [];
        if (!forLanguage.Contains(source, StringComparer.Ordinal)) forLanguage.Add(source);
    }

    /// <summary>Everything declared for this language, ready to paste above a wrapper.</summary>
    /// <remarks>
    /// ⚠ Part of the shim's IDENTITY as well as its text — see Entry. Two programs whose axiom is
    /// byte-identical but whose preambles differ are different shims, and caching on the axiom
    /// alone would hand the second one the first one's object file.
    /// </remarks>
    private string PreambleFor(string language) =>
        _preambles.TryGetValue(language, out var forLanguage)
            ? string.Concat(forLanguage.Select(src => ForeignC.Preamble(language, src)))
            : "";
    ...
    private LoadedShim Entry(string language, string source,
                             IReadOnlyList<(CufetType Type, string Name)> parameters,
                             CufetType result, int line) =>
        // ⚠ The preamble is part of the key. The on-disk cache hashes the whole shim text so it was
        // already safe; this one keyed on the axiom alone, which is only correct while every
        // preamble is declared before anything is prepared. Making it true here means it does not
        // depend on that ordering being remembered.
        _loaded.GetOrAdd(ForeignC.Identity(language, source, parameters, result)
                       + "\0" + PreambleFor(language),
    ...
    private string Build(string language, string source,
                         ...
                         CufetType result, int line)
    {
        string shim = ShimSource(language, source, parameters, result, PreambleFor(language));
    ...
    private static string ShimSource(string language, string source,
                                     IReadOnlyList<(CufetType Type, string Name)> parameters,
                                     CufetType result, string preamble)
    {
        ...
        // ★★ Resultless axioms, above the wrapper — the same order and the same reason the compiled
        // backend uses: the wrapper may name a helper one of them declares.
        if (preamble.Length > 0) { sb.Append(preamble); sb.AppendLine(); }
